...
export interface InitialClienti {
  contacts: { id: string; firstName: string; lastName: string; email: string; phone: string; companyName: string; dateAdded: string; tags: string[] }[]
  stats: { total: number; withEmail: number; withPhone: number; newThisMonth: number }
  hasMore: boolean
  bookingCounts: Record<string, number>
  lastBooking: Record<string, string>
}
...
export default function ClientiView({ initial }: { initial?: InitialClienti }) {
  const [contacts, setContacts] = useState<Contact[]>(initial?.contacts ?? [])
  const [bookingCounts, setBookingCounts] = useState<Record<string, number>>(initial?.bookingCounts ?? {})
  const [lastBooking, setLastBooking] = useState<Record<string, string>>(initial?.lastBooking ?? {})
  const [stats, setStats] = useState(initial?.stats ?? { total: 0, withEmail: 0, withPhone: 0, newThisMonth: 0 })
  ...
  // Skip the first fetch when the server already provided the initial page.
