...
// ── Page ──────────────────────────────────────────────────────────────────
export interface InitialCalendario {
  events: CalEvent[]
  users: GhlUser[]
  calendars: GhlCalendar[]
}

export default function CalendarioView({ initial }: { initial?: InitialCalendario }) {
  const [viewMode, setViewMode] = useState<ViewMode>('week')
  const [anchor, setAnchor] = useState(() => new Date())
  const [events, setEvents] = useState<CalEvent[]>(initial?.events ?? [])
  const [users, setUsers] = useState<GhlUser[]>(initial?.users ?? [])
  const [calendars, setCalendars] = useState<GhlCalendar[]>(initial?.calendars ?? [])
  ...
  useEffect(() => {
    if (initial?.users?.length || initial?.calendars?.length) return
  ...
  // Skip the first fetch when the server seeded the current week's events.
  const seededEvents = useRef(!!initial)
  useEffect(() => {
    if (seededEvents.current) { seededEvents.current = false; return }
    setLoading(true)
