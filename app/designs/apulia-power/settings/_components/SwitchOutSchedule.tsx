...
import { setSwitchOutDate, setSwitchOutDatesBulk } from '../../condomini/[id]/_actions'

export interface SwitchOutEntry {
  contactId: string
  pod: string
  cliente: string
...
export default function SwitchOutSchedule({ pods }: { pods: SwitchOutEntry[] }) {
  ...
  const filtered = useMemo(() => {
    let out = pods
    if (filter === 'unset') out = out.filter((p) => !p.switchedOutAt)
  ...
  function applyBulk() {
    ...
    startBulkTransition(async () => {
      const res = await setSwitchOutDatesBulk(Array.from(selected), bulkDate)
    ...
    <div>
      ...
      {selected.size > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--ap-bg-muted, #f1f5f9)', border: '1px solid var(--ap-line)', borderRadius: 10, marginBottom: 12, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 13, fontWeight: 700 }}>{selected.size} POD selezionati</span>
          <div style={{ flex: 1 }} />
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
            <span style={{ color: 'var(--ap-text-muted)' }}>Imposta switch-out:</span>
            <input
              type="date"
              value={bulkDate}
          ...
          <button type="button" onClick={applyBulk} disabled={!bulkDate || bulkPending} className="ap-btn ap-btn-primary" style={{ height: 30, fontSize: 12 }}>
            {bulkPending ? 'Applico…' : 'Applica a selezionati'}
          </button>
          <button type="button" onClick={() => setSelected(new Set())} className="ap-btn ap-btn-ghost" style={{ height: 30, fontSize: 12 }}>Annulla</button>
...
function Row({ pod, selected, onToggle }: { pod: SwitchOutEntry; selected: boolean; onToggle: () => void }) {
  const [pending, startTransition] = useTransition()
  const [val, setVal] = useState<string>(pod.switchedOutAt ? pod.switchedOutAt.slice(0, 10) : '')
  ...
  function commit() {
    ...
    startTransition(async () => {
      const res = await setSwitchOutDate(pod.contactId, val)
