...
export async function POST(req: NextRequest) {
  ...
  const {
    ...
    calendarId, serviceName, operatorId,
    preferredDate, timePref, preferredFrom, preferredTo, note,
  ...
  if (!preferredDate) return NextResponse.json({ error: 'Giorno preferito obbligatorio' }, { status: 400 })
  ...
  const tp = ['any', 'morning', 'afternoon', 'specific'].includes(timePref) ? timePref : 'any'
  const { error } = await sb.from('bellessere_waitlist').insert({
    ...
    time_pref: tp,
    preferred_from: tp === 'specific' ? (preferredFrom || null) : null,
    preferred_to: tp === 'specific' ? (preferredTo || null) : null,
    note: note ?? null,
    status: 'waiting',
  })
